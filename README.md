...
- `app/` - Flask API server
	...
	- `main.py` - CSV ingestion and cleaning (z-score based outlier removal) that produces `output_data/cleaned.csv` and inserts cleaned records into MongoDB
...
Notes:
- `MONGODB_URI` should be the host portion (without `mongodb+srv://`) when using the SRV connection method constructed inside `app/app.py`.
...
All responses are JSON. The API expects cleaned data inserted by `app/main.py` or your own dataset inserted into the `water_quality_data.asv_1` collection in MongoDB.
